...
```bash
./tools/train_net.py --config-file ./configs_old/Market1501/bagtricks_R50.yml MODEL.DEVICE "cuda:0"
```
...
```bash
python tools/train_net.py --config-file ./configs_old/Market1501/bagtricks_R50.yml --num-gpus 4
```

To evaluate a model's performance, use

```bash
python tools/train_net.py --config-file ./configs_old/Market1501/bagtricks_R50.yml --eval-only \
