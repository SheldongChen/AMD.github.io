...
class SimpleTrainer(TrainerBase):
    ...
    #TODO CXD
    def unused_param_sum_zero(self,outs):
        unused_param = None
        for k,v in outs.items():
            if unused_param==None:
                if isinstance(v,dict):
                    unused_param = self.unused_param_sum_zero(v)
                elif isinstance(v,list):
                    for list_v in v:
                        if unused_param==None:
                            unused_param = (torch.zeros_like(list_v) * list_v).sum()
                        else:
                            unused_param += (torch.zeros_like(list_v) * list_v).sum()
    ...
    def run_step(self):
        ...
        with amp.autocast(enabled=self.amp_enabled):
            ...
            losses = sum(loss_dict.values()) + self.unused_param_sum_zero(outs)
