...
class FreezeLayer(HookBase):
    def __init__(self, model, optimizer, freeze_layers, freeze_iters,interpretation_freeze_layers):
        self._logger = logging.getLogger(__name__)
        ...
        self.interpretation_freeze_layers = interpretation_freeze_layers
    ...
    def before_step(self):
        ...
        if self.trainer.iter > self.freeze_iters and self.is_frozen:
            ...
            self.open_all_layer()
            self.freeze_layers = self.interpretation_freeze_layers
            if self.interpretation_freeze_layers!= ['']:
              self.freeze_specific_layer()
