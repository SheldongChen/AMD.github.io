...
class ADD_BNneckHead(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        # fmt: off
        in_feat        = cfg.MODEL.HEADS.IN_FEAT

        #num_classes = cfg.INTERPRETATION.NUM_ATT
    ...
    def forward(self, features, targets=None,training=None):
        """
        See :class:`ReIDHeads.forward`.
        """
        #Forced the self.training = training
        if training:
            training_copy = self.training
        ...
        if not self.training:
            # get self.training back
            if training:
                self.training = training_copy
            return bn_feat
        # fmt: on
