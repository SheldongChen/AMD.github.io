...
class Checkpointer(object):
    ...
    def load_n(self,path:str,cfg):
        ...
        #print("_strip_prefix_if_present(checkpoint_state_dict, )")

        # change the key of Dict
        ...
        if cfg.INTERPRETATION.MODEL.SHARE_LAYER==0:
            ...
            for k,v in checkpoint_state_dict.items():
                if After_backbone:
                    k = k.replace("backbone", "backbone_2")
                else:
                    k = k.replace("backbone","backbone_1")


                new_checkpoint_state_dict[k] = v
        elif 1<=cfg.INTERPRETATION.MODEL.SHARE_LAYER<=4:
            After_backbone = False
            # "backbone.layer"+str()
            for k, v in checkpoint_state_dict.items():
                if not After_backbone and "backbone.layer"+str(cfg.INTERPRETATION.MODEL.SHARE_LAYER) in k:
                    After_backbone=True
            ...
            # "backbone.layer"+str()
            for k, v in checkpoint_state_dict.items():
                if not After_backbone and "heads." in k:
                    After_backbone = True
        ...
        self.logger.info(
            " k in list(checkpoint_state_dict.keys())"
        )
        #print(" k in list(checkpoint_state_dict.keys())")
